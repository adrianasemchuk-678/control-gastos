...
if opcion == "💰 Mi Presupuesto & Panel":
    ...
    with col_i1:
        nuevo_ingreso = st.number_input(
            "Ingreso / Sueldo del Mes ($):",
            min_value=0.0,
            value=float(usr_data.get("ingreso_inicial", 0.0)),
            step=10000.0,
        )
    ...
    with col_i2:
        alcancia_val = st.number_input(
            "🐷 Alcancía de Ahorros ($):",
            min_value=0.0,
            value=float(usr_data.get("alcancia", 0.0)),
            step=5000.0,
        )
    ...
    total_fijos_pagados = sum(
        pf["monto"] for pf in usr_data["pagos_fijos"] if pf["pagado"]
    )
    ...
    if usr_data["meta_alcancia"] > 0:
        ...
        st.progress(
            progreso,
            text=f"Llevas un {progreso*100:.1f}% alcanzado ({fmt_moneda(usr_data['alcancia'])} de {fmt_moneda(usr_data['meta_alcancia'])})",
        )
...
# --- SECCIÓN 4: PREFERENCIAS ---
    ...
    nom_m = st.text_input(
        "Objetivo de Ahorro (ej. Viaje, Cambio de Auto)",
        value=usr_data["nombre_meta"],
    )
    val_m = st.number_input(
        "Monto Meta ($):",
        min_value=0.0,
        value=float(usr_data["meta_alcancia"]),
        step=10000.0,
    )
    ...
    usar_p = st.toggle(
        "Activar límites mensuales por categoría",
        value=usr_data["usar_presupuestos"],
    )
...
# --- SECCIÓN 5: REPORTES Y EXPORTACIONES ---
    ...
    if es_admin and sel == "Todos":
        ...
        for u, udata in db_data.items():
            ...
            for pf in udata.get("pagos_fijos", []):
                if pf["pagado"]:
                    gastos_totales_lista.append({
                        "fecha": "Pago Fijo",
                        "categoria": pf["concepto"],
                        "monto": pf["monto"],
                        "usuario": u,
                    })
    else:
    ...
    if gastos_totales_lista:
        ...
        st.dataframe(
            df_rep_display.drop(columns=["monto"]), use_container_width=True
        )
        ...
        cat_chart.plot(
            ...
            autopct="%1.1f%%",
            ax=ax,
            colors=["#FFB6C1", "#87CEFA", "#98FB98", "#DDA0DD", "#F0E68C", "#FFD700"],
        )
        ax.set_ylabel("")
        ...
        with pd.ExcelWriter(excel_buf, engine="openpyxl") as writer:
            ...
            df_rep_excel.drop(columns=["monto"]).to_excel(
